VWFMVIEW ; WAA/AA - FMVIEW GUI INTERFACE ROUTINE ; AUG 23, 2012@11:54
 ;;1.43;ZZFMVIEW;;Jan 10, 2025@13:31;;
 ;
...
ZRCHKSUM(NAME) ;Returns checksum of routine 
 ...
 I '$T S R=X_"^?"
 Q R
LISTXIDX ;;; Return XINDEX output for a routine
 ; Input:
 ;   ARRAY(0) = routine name (e.g. "DGPMV")
 ; Output:
 ;   RESULT(n) = n|text of XINDEX output
 N RTN,OUT,I,LINE,IDX
 K RESULT,^TMP($J,"XINDEX")
 S RTN=$G(ARRAY(0))
 I RTN="" S RESULT(0)="-1^No routine name provided" Q
 ; Run XINDEX and redirect output to ^TMP($J,"XINDEX")
 ; EN^XINDEX takes: routine name, output global root
 D EN^XINDEX(RTN,"^TMP($J,""XINDEX"")")
 ; If nothing returned, report error
 I '$D(^TMP($J,"XINDEX")) S RESULT(0)="-1^No XINDEX output" Q
 ; Copy XINDEX output into RESULT array
 S I=0,IDX=0
 S LINE=""
 F  S IDX=$O(^TMP($J,"XINDEX",IDX)) Q:'IDX  D
 . S LINE=$G(^TMP($J,"XINDEX",IDX))
 . S I=I+1
 . S RESULT(I)=I_"|"_LINE
 ; First node: count of lines
 S RESULT(0)=I_"|XINDEX output for routine "_RTN
 Q 